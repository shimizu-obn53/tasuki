...
export default function Home() {
  const recentListings = listings.filter(l => l.status === '公開中').slice(0, 3)
            ...
            {recentListings.map((listing) => (
              <Link href={`/listings/${listing.id}`} key={listing.id}>
                <div className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-md transition border border-gray-100 cursor-pointer">
                  ...
                  <p className="text-sm text-gray-500">月商 {listing.monthlyRevenue}万円 / 創業{listing.yearsInBusiness}年</p>
                </div>
              </Link>
            ))}
          </div>
